...
import { exportToCSV, exportToMarkdown, exportToPDF } from './services/exportService';
import { FileText, Download, ShieldCheck, ChevronRight, CheckCircle2, AlertTriangle, Menu, X, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleFillExample = () => {
    if (Object.keys(formData).length > 0) {
      if (!window.confirm('这将覆盖当前所有已填写的内容，确定要填入示例数据吗？')) {
        return;
      }
    }
  ...
  const isFormValid = useMemo(() => {
    return Object.values(sectionProgress).every((p: { complete: boolean }) => p.complete);
  }, [sectionProgress]);
  ...
  const handleExport = (format: 'csv' | 'md' | 'pdf') => {
    if (!isFormValid) {
      setShowErrors(true);
      alert('请先完成所有必填项再导出 (Please complete all required fields before exporting)');
      return;
    }
    
    switch (format) {
      case 'csv': exportToCSV(formData); break;
      case 'md': exportToMarkdown(formData); break;
      ...
      `}>
        <div className="h-full flex flex-col">
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {stats.complete ? (
                      ...
                      <span className="text-xs text-slate-400">{stats.filled}/{stats.total}</span>
                    )}
                  </div>
          ...
          <div className="p-4 bg-slate-50 border-t space-y-4">
            ...
            <div>
              <div className="text-xs text-slate-500 mb-2 font-medium">导出选项 (Export)</div>
        ...
        <div className="max-w-3xl mx-auto p-6 md:p-12 pb-32">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 md:p-8">
            ...
            <div className="flex justify-between mt-8 pt-6 border-t">
              ...
              >
                {SECTIONS.findIndex(s => s.id === activeSection) === SECTIONS.length - 1 ? '完成检查' : '下一步'}
                {SECTIONS.findIndex(s => s.id === activeSection) !== SECTIONS.length - 1 && <ChevronRight size={16} className="ml-1" />}
              </button>
            </div>
          </div>
